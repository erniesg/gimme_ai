...
async function handleGenerateVideo(request, env, workflowClass, apiBaseUrl) {
  try {
    ...
    console.log("Video generation handler called");
    console.log("Workflow class binding:", workflowClass);
    ...
    if (!env[workflowClass]) {
      console.error(`ERROR: Workflow binding '${workflowClass}' not found`);
      return new Response(JSON.stringify({
        error: "Workflow binding not found",
        binding_name: workflowClass,
    ...
    console.log(`Creating workflow with binding: ${workflowClass}`);
    console.log("Creating with workflow_type: video");

    // Create workflow instance with workflow_type='video'
    ...
    console.log("Workflow instance created with ID:", workflowInstance.id);
...
export default {
  async fetch(request, env) {
    ...
    // Calculate the workflow class name from project name
    const projectName = env.PROJECT_NAME || "{{ project_name }}";
    const workflowClass = `${projectName.toUpperCase().replace(/-/g, '_')}_WORKFLOW`;

    // Get backend API URL
    const apiBaseUrl = env.MODAL_ENDPOINT || "{{ endpoints.prod }}";

    console.log("Video workflow handler received path:", path);
